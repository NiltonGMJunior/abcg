...
void OpenGLWindow::initializeGL() {
  ...
  glBufferData(
                GL_ARRAY_BUFFER, sizeof(m_vertices[0]) * m_vertices.size(),
                m_vertices.data(),
                GL_STATIC_DRAW
              );
  ...
  glBufferData(
                GL_ELEMENT_ARRAY_BUFFER, sizeof(m_indices[0]) * m_indices.size(),
                m_indices.data(),
                GL_STATIC_DRAW
              );
...
void OpenGLWindow::loadModelFromFile(std::string_view path) {
  ...
  readerConfig.mtl_search_path = getAssetsPath() + "mtl/"; // Path to material files

  tinyobj::ObjReader reader;

  if (!reader.ParseFromFile(path.data(), readerConfig)) {
    if (!reader.Error().empty()) {
      throw abcg::Exception{abcg::Exception::Runtime(fmt::format("Failed to load model {} ({})", path, reader.Error()))};
    }
  }
  ...
  // A key:value map wit key=Vertex and value=index
  ...
  for (const auto& shape : shapes) {
    ...
    for (const auto faceNumber : iter::range(shape.mesh.num_face_vertices.size())) {
      // Number pf vertices composing face f
...
void OpenGLWindow::standardize() {
  ...
  for (const auto& vertex : m_vertices) {
    ...
    min.y = std::min(min.x, vertex.position.y);
